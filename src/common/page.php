 ...
 * $page：分页get参数
 */
function page($count, $page_size, $num_btn = 5, $page = 'page') {
	/*
	 * 判断是否有记录
	 */
	if ($count==0){
	...
	if ($num_btn >= $page_num_all) {
		...
		for($i = 0; $i < $num_btn; $i ++) {
			if ($_GET [$page] == $start) {
				$html [$start] = "<li class='disabled'><a href='#'>{$start}</a></li>";
			} else {
	...
	if ($_GET [$page] != 1) {
		$prev = $_GET [$page] - 1;
		array_unshift ( $html, "<li><a href='{$url}{$prev}' aria-label='Previous'>&laquo;&nbsp;上一页</a></li>" );
	}
	if ($_GET [$page] != $page_num_all) {
		$next = $_GET [$page] + 1;
		array_push ( $html, " <li><a href='{$url}{$next}'  aria-label='Next'>下一页&nbsp;&raquo;</a></li>" );
	}
	$html = implode ( '  ', $html );
	$data = array (
			'limit' => $limit,
